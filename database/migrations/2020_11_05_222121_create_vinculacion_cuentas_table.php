...
class CreateVinculacionCuentasTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('vinculacion_cuenta', function (Blueprint $table) {
            ...
            $table->foreignId('id_cuenta')->constrained('cuenta')->onUpdate('cascade')->onDelete('cascade');
            $table->foreignId('id_cuenta_sistema')->constrained('cuenta_sistema')->onUpdate('cascade')->onDelete('cascade');
            $table->foreignId('id_empresa')->constrained('empresa')->onUpdate('cascade')->onDelete('cascade');
            $table->timestamps();
            $table->primary(['id_cuenta', 'id_cuenta_sistema', 'id_empresa'], 'vc_id_ccse_primary');
        });
    }
