...
class CuentaPeriodo extends Migration
{
    ...
    public function up()
    {
        Schema::create('cuenta_periodo', function (Blueprint $table) {
            ...
            $table->foreignId('cuenta_id')->constrained('cuenta')->onUpdate('cascade')->onDelete('cascade');
            $table->foreignId('periodo_id')->constrained('periodo')->onUpdate('cascade')->onDelete('cascade');
            $table->timestamps();
        });
    }
